document.addEventListener('DOMContentLoaded', () => {
  ...
  // ✅ Master Sheet Script URL
  const MASTER_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbxRC0ik1RcrMdE4DwDt2F9VVHYE3C4TADMuI7_nQf-WnTefXj00YDOmpghaEnQnlzOR0g/exec";
  ...
  submitToSheetsBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      // 🔹 1) Send to District Sheet
      const res = await fetch(url, { method: 'POST', body: body });
      const data = await res.json();

      // 🔹 2) Also Send to Master Sheet (without disturbing anything)
      try {
        await fetch(MASTER_SCRIPT_URL, { method: 'POST', body: body });
        console.log("Data also sent to Master Sheet successfully");
      } catch (mErr) {
        console.error("Error sending to Master Sheet:", mErr);
      }
